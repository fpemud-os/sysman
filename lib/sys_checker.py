...
class FmSysChecker:
    ...
    def _checkAndGetStorageLayout(self):
        ...
        with self.infoPrinter.printInfoAndIndent("- Checking swap service"):
            ...
            if layout.name in ["bios-ext4", "efi-ext4", "efi-btrfs", "efi-bcache-btrfs", "efi-bcachefs"]:
    ...
    def _checkPreMountRootfsLayout(self, layout):
        mRootfs = [m for m in layout.get_mount_entries() if m.mountpoint == "/"][0]
        with TmpMount(mRootfs.target, options=mRootfs.mnt_opts) as mp:
            obj = strict_fsh.PreMountRootFs(mp.mountpoint,
                                            ...
                                            mounted_etc=False,
                                            mounted_home=any([m.mountpoint == "/root" for m in layout.get_mount_entries()]),
                                            mounted_var=any([m.mountpoint.startswith("/var/") for m in layout.get_mount_entries()]))
